...
import { CustomerData, Document, DocumentData, Invoice, ItemList, ItemsList, ProductSummaryUiData, userData } from './invoice.model';
@Injectable({
  providedIn: 'root'
})
export class InvoicesService {
  fieldNameEstimate: string;//store fieldname of estimate
  fieldNameQuotation: string;//store fieldname of quotation
  ...
  invoice: Invoice;// for saving document
  ...
  invoiceAutoPayLinkEnable: boolean; //for check payment link enable or not
  rzrAccountId: string;
  ...
  invoiceOrgTag: boolean = false;//for check org tag required
  invoiceContactTag: boolean = false;//for check contact tag required
  invoiceSaleTag: boolean = false;//for check sale tag required
  daTime: any;
  ...
  newProduct: boolean = false; //flag indicates addition of new product
  ...
  removeProduct(index) {
    ...
    if(this.invoice.itemList[index].item){
      this.deletedProducts[Object.keys(this.deletedProducts).length] = {
        'item': this.invoice.itemList[index].item,
        'index': index
      };
    }
    this.document.itemList.splice(index, 1);
    this.invoice.itemList.splice(index, 1);
  ...
  updateValues() {
    this.document.itemList.forEach((data, index) => {
      ...
      this.invoice.itemList[index].amount = this.document.itemList[index].totalProductRate;
      this.invoice.itemList[index].amountInclTax = this.document.itemList[index].amountInclTax;
      this.invoice.itemList[index].description = this.document.itemList[index].description
      this.invoice.itemList[index].hsnCode = this.document.itemList[index].hsnCode
      this.invoice.itemList[index].item = this.document.itemList[index].item
      this.invoice.itemList[index].qty = this.document.itemList[index].qty
      this.invoice.itemList[index].rate = this.document.itemList[index].rate
      this.invoice.itemList[index].slno = index
      this.invoice.itemList[index].unit = this.document.itemList[index].unit


      this.invoice.itemList[index].cessAmount = this.document.itemList[index].cessAmount
      if (this.invoice.itemList[index].cessAmount == 0) {
        this.invoice.itemList[index].cessRate = 0
      } else {
        this.invoice.itemList[index].cessRate = this.document.itemList[index].cessPercentage
      }
      this.invoice.itemList[index].cgstAmount = this.document.itemList[index].cgstAmount
      if (this.invoice.itemList[index].cgstAmount == 0) {
        this.invoice.itemList[index].cgstRate = 0
      } else {
        this.invoice.itemList[index].cgstRate = this.document.itemList[index].cgstPercentage
      }
      this.invoice.itemList[index].discountedAmount = this.document.itemList[index].amountDiscounted
      if (this.invoice.itemList[index].discountedAmount == 0) {
        this.invoice.itemList[index].discountAmount = 0
        this.invoice.itemList[index].discountRate = 0
      } else {
        this.invoice.itemList[index].discountAmount = this.document.itemList[index].rateAfterDiscount
        this.invoice.itemList[index].discountRate = this.document.itemList[index].discountPercentage
      }


      this.invoice.itemList[index].igstAmount = this.document.itemList[index].igstAmount
      if (this.invoice.itemList[index].igstAmount == 0) {
        this.invoice.itemList[index].igstRate = 0
      } else {
        this.invoice.itemList[index].igstRate = this.document.itemList[index].igstPercentage
      }


      this.invoice.itemList[index].sgstAmount = this.document.itemList[index].sgstAmount
      if (this.invoice.itemList[index].sgstAmount == 0) {
        this.invoice.itemList[index].sgstRate = 0

      } else {
        this.invoice.itemList[index].sgstRate = this.document.itemList[index].sgstPercentage

      }

      this.invoice.itemList[index].vatAmount = this.document.itemList[index].vatAmount
      if (this.invoice.itemList[index].vatAmount == 0) {
        this.invoice.itemList[index].vatRate = 0
      } else {
        this.invoice.itemList[index].vatRate = this.document.itemList[index].vatPercentage
      }
    ...
    this.invoice.docData.cessValue = this.document.summaryData.cessValue;
    this.invoice.docData.cgstValue = this.document.summaryData.cgstValue;
    this.invoice.docData.discountValue = this.document.summaryData.totalAmountDiscounted
    this.invoice.docData.currency = this.document.billingAmountDetails.currency;
    this.invoice.docData.discountedAmount = this.document.summaryData.totalAfterDiscount
    this.invoice.docData.igstValue = this.document.summaryData.igstValue
    this.invoice.docData.includeCess = this.billingAmountDetailsForm.value.includeCess
    this.invoice.docData.includeDiscount = this.billingAmountDetailsForm.value.includeDiscount
    this.invoice.docData.includeTax = this.billingAmountDetailsForm.value.includeTax
    this.invoice.docData.includeUnit = this.billingAmountDetailsForm.value.includeUnit
    this.invoice.docData.interState = this.billingAmountDetailsForm.value.interState
    this.invoice.docData.sgstValue = this.document.summaryData.sgstValue
    this.invoice.docData.taxType = this.billingAmountDetailsForm.value.taxType
    this.invoice.docData.total = this.document.summaryData.total
    this.invoice.docData.totalInclTax = this.document.summaryData.totalInclTax
    this.invoice.docData.vatValue = this.document.summaryData.vatValue;
  ...
  getPrefixAndDocNumber(prefix, invNoLast) {
    if (!prefix) {
      prefix = '';
    }
    let docNumber = 0;
    ...
    let num = docNumber + 1
    this.invoice.docData.docNumber = num.toString()
    this.invoice.docData.docPrefix = prefix
    this.invoice.docData.prefixAndDocNumber = prefix + (docNumber + 1)
  }
  ...
  updateFormDetailsFromUserdata(allData: UserDatas) {
    ...
    this.fieldNameQuotation = allData.superUserDetails.fieldNames?.fieldNameQuotation ? allData.superUserDetails.fieldNames?.fieldNameQuotation : 'Quotation'
    ...
    this.invoiceAutoPayLinkEnable = (!!allData.superUserDetails.invoiceAutoPayLinkEnable)
    ...
    if (this.scenario == 'create' &&  !this.formResetDisable) {
      ...
      this.invoice.createdBy = allData.userId;
      this.mainForm.patchValue({ docTitle: allData.superUserDetails.fieldNames?.fieldNameInvoice ? allData.superUserDetails.fieldNames?.fieldNameInvoice : 'Invoice' })
      this.docDetailsForm.patchValue({ docTitle: allData.superUserDetails.fieldNames?.fieldNameInvoice ? allData.superUserDetails.fieldNames?.fieldNameInvoice : 'Invoice' })

      this.getPrefixAndDocNumber(allData.superUserDetails.invoiceNumberPrefix, allData.superUserDetails.invoiceNoLast)
      this.createItemFormGroupDefault();
      if (allData.superUserDetails?.invoiceApproval) {
        this.invoice.docData.statusApproved = false;
      } else {
        this.invoice.docData.statusApproved = true;
      }
      if (this.custID == 'none') {
        this.custID = null
  ...
  updateEditalue(data) {
    this.invoice.docData.docTitle = data.docData?.docTitle;
    this.invoice.createdBy = data?.createdBy;
    this.invoice.sharedDocId = data?.sharedDocId;
    this.invoice.collectedAmount = data?.collectedAmount;
  ...
  // in create inv from estimate/quo scenarion bind basic details
  updateInoviceEditalue(data, allData,) {
    this.saleID = data.docData?.saleID; // bind sale id
    this.custID = data.customerData?.custID; // bind customer id
    this.orgID = data.customerData?.orgID; // bind orgID 
    this.userData = data.userData;
    ...
    this.docData.docType = 'Invoice'; //reset the docType to Invoice
    if (this.scenario == 'invfromest') {
      this.docData.estRef = data.docData.prefixAndDocNumber;
      this.invoice.docData.estRef = data.docData.prefixAndDocNumber;
    }
    if (this.scenario == 'invfromquote') {
      this.docData.quoteRef = data.docData.prefixAndDocNumber;
      this.invoice.docData.quoteRef = data.docData.prefixAndDocNumber;
    }
    ...
    this.docData.docTitle = allData.superUserDetails.fieldNames?.fieldNameInvoice ? allData.superUserDetails.fieldNames?.fieldNameInvoice : 'Invoice'
    this.getPrefixAndDocNumber(allData.superUserDetails.invoiceNumberPrefix, allData.superUserDetails.invoiceNoLast)
  ...
  formValueChanges() {
    ...
    this.mainForm.valueChanges.subscribe((data) => {
      this.invoice.docData.docTitle = this.mainForm.value.docTitle
      this.updateValidCheck()
    })
  ...
  bindAddtionalDetails() {
    ...
    this.invoice.additionalFieldsArr = []
    this.invoice.additionalFieldsArr = additionalFields
  }
  bindCustomerData() {
    this.invoice.customerData.addressline1 = this.billToForm.value.billToAddressline1;
    this.invoice.customerData.addressline2 = this.billToForm.value.billToAddressline2;
    this.invoice.customerData.companyName = this.billToForm.value.billToCompanyName;
    this.invoice.customerData.contactNumber = this.billToForm.value.billToContactNumber ? this.billToForm.value.billToContactNumber + '' : this.billToForm.value.billToContactNumber;
    this.invoice.customerData.country = this.billToForm.value.billToCountry;
    this.invoice.customerData.countryCode = this.billToForm.value.billToCountryCode;
    this.invoice.customerData.district = this.billToForm.value.billToDistrict;
    this.invoice.customerData.email = this.billToForm.value.billToEmail;
    this.invoice.customerData.fname1 = this.billToForm.value.billToFname1;
    this.invoice.customerData.gst = this.billToForm.value.billToGst;
    this.invoice.customerData.pinCode = this.billToForm.value.billToPinCode;
    this.invoice.customerData.sname = this.billToForm.value.billToSname;
    this.invoice.customerData.state = this.billToForm.value.billToState;
    this.invoice.customerData.surname = this.billToForm.value.billToSurname;
    this.invoice.searchTerm.firstName = this.billToForm.value.billToFname1 ? this.billToForm.value.billToFname1.toLowerCase() : ''
    this.invoice.searchTerm.secondName = this.billToForm.value.billToSname ? this.billToForm.value.billToSname.toLowerCase() : ''
    this.invoice.searchTerm.surname = this.billToForm.value.billToSurname ? this.billToForm.value.billToSurname.toLowerCase() : ''
    this.invoice.searchTerm.companyName = this.billToForm.value.billToCompanyName ? this.billToForm.value.billToCompanyName.toLowerCase() : ''
  }
  bindDeliveryData() {
    this.invoice.customerData.isDeliveryAddressPresent = this.deliverdToForm.value.isDeliveryAddressPresent;
    this.invoice.customerData.deliveryAddressline1 = this.deliverdToForm.value.deliveryAddressline1;
    this.invoice.customerData.deliveryAddressline2 = this.deliverdToForm.value.deliveryAddressline2;
    this.invoice.customerData.deliveryContactName = this.deliverdToForm.value.deliveryContactName;
    this.invoice.customerData.deliveryCompanyName = this.deliverdToForm.value.deliveryCompanyName;
    this.invoice.customerData.deliveryEmail = this.deliverdToForm.value.deliveryEmail;
    this.invoice.customerData.deliveryContactNumber = this.deliverdToForm.value.deliveryContactNumber ? this.deliverdToForm.value.deliveryContactNumber + '' : this.deliverdToForm.value.deliveryContactNumber;
    this.invoice.customerData.deliverycountryCode = this.deliverdToForm.value.deliverycountryCode;
    this.invoice.customerData.deliveryCountry = this.deliverdToForm.value.deliveryCountry;
    this.invoice.customerData.deliveryDistrict = this.deliverdToForm.value.deliveryDistrict;
    this.invoice.customerData.deliveryPinCode = this.deliverdToForm.value.deliveryPinCode;
    this.invoice.customerData.deliveryState = this.deliverdToForm.value.deliveryState;
  }
  bindUserAddressData(data) {
    this.invoice.userData.addressline1 = this.billFromForm.value.billFromAddressline1;
    this.invoice.userData.addressline2 = this.billFromForm.value.billFromAddressline2;
    this.invoice.userData.city = this.billFromForm.value.billFromCity;
    this.invoice.userData.companyName = this.billFromForm.value.billFromCompanyName;
    this.invoice.userData.contactname = this.billFromForm.value.billFromContactname;
    this.invoice.userData.country = this.billFromForm.value.billFromCountry;
    this.invoice.userData.gst = this.billFromForm.value.billFromGst;
    this.invoice.userData.pinCode = this.billFromForm.value.billFromPinCode;
    // this.invoice.userData.secondName = this.billFromForm.value.secondName;
    this.invoice.userData.state = this.billFromForm.value.billFromState;
  }
  bindUserData() {
    this.invoice.userData.contactno = this.signatureAndAdditionalDetailsForm.value.signatoryContactno ? this.signatureAndAdditionalDetailsForm.value.signatoryContactno + '' : this.signatureAndAdditionalDetailsForm.value.signatoryContactno;
    this.invoice.userData.designation = this.signatureAndAdditionalDetailsForm.value.designation;
    this.invoice.userData.email = this.signatureAndAdditionalDetailsForm.value.signatoryEmail;
    this.invoice.userData.signatoryName = this.signatureAndAdditionalDetailsForm.value.signatoryName;
    this.invoice.docData.bankDetails = this.signatureAndAdditionalDetailsForm.value.bankDetails;
    this.invoice.docData.notes = this.signatureAndAdditionalDetailsForm.value.notes;
  }
  bindDocData() {
    this.invoice.docData.docDate = this.docDetailsForm.value.docDate;
    this.invoice.docData.dueDate = this.docDetailsForm.value.dueDate;
    this.invoice.docData.gstPlaceOfSupplyCode = this.docDetailsForm.value.gstPlaceOfSupplyCode;
    this.invoice.docData.gstStateCode = this.docDetailsForm.value.gstStateCode;
    this.invoice.docData.poRef = this.docDetailsForm.value.poRef;
    this.invoice.docData.paymentTerm = this.docDetailsForm.value.paymentTerm;
  }
  ...
  getQuotationDocumentDetails(userId, docId) {
    return this.firestore
  ...
  createDocument(changeLog) {
    return this.firestore
      .collection('users/' + this.superUserId + '/Invoices/')
      .add({
  ...
  editDocument(changeLog) {
    if(Object.keys(changeLog).length > 0)
      return this.firestore.collection('users/' + this.superUserId + '/Invoices/')
        .doc(this.docID)
        .set({
          ...this.invoice, changeLog
        })
  ...
  addinvoicetoshare(saleId, docnumber, doc) {
    return this.firestore
      .doc('shared/' + saleId + '/' + doc + '/' + docnumber)
      .set({ docNumber: docnumber, shareDate: Date.now() });
  }
  initshareinvoice(data) {
    return this.firestore.doc('shared/' + data.saleID).set(data);
  }
  togglesharestatus(userid, docNumber, shareStatus, doc) {
    return this.firestore
      .doc('users/' + userid + '/' + doc + '/' + docNumber)
