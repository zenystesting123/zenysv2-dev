...
import { CustomerData, Document, DocumentData, Quotation, ItemList, ItemsList, ProductSummaryUiData, userData } from './quotation.model';
@Injectable({
  providedIn: 'root'
})
export class QuotationService {
  fieldNameEstimate: string;//store fieldname of estimate
  ...
  docType: string;// store doc type
  scenario: string; // scenario for the doc(create,edit,view)
  disableSaveButton: boolean = true; // for disable save button
  ...
  quotation: Quotation;// for saving document
  ...
  quotationAutoPayLinkEnable: boolean; //for check payment link enable or not
  rzrAccountId: string;
  ...
  quoteOrgTag: boolean = false; //for check org tag required
  quoteContactTag: boolean = false;//for check contact tag required
  quoteSaleTag: boolean = false;//for check org sale required
  daTime: any;
  ...
  newProduct: boolean = false; //flag indicated addition of new product
  ...
  removeProduct(index) {
    ...
    if(this.quotation.itemList[index].item){
      this.deletedProducts[Object.keys(this.deletedProducts).length] = {
        'item': this.quotation.itemList[index].item,
        'index': index
      };
    }
    this.document.itemList.splice(index, 1);
    this.quotation.itemList.splice(index, 1);
  ...
  getDataFromUI() {
    ...
    if (this.document.billingAmountDetails.taxType == 'vat') {
      ...
      this.docDetailsForm.get('gstPlaceOfSupplyCode').markAsDirty();
      this.docDetailsForm.get('gstStateCode').markAsDirty();
    }
    this.updateValues();
  }
  ...
  updateValues() {
    this.document.itemList.forEach((data, index) => {
      ...
      this.quotation.itemList[index].amount = this.document.itemList[index].totalProductRate;
      this.quotation.itemList[index].amountInclTax = this.document.itemList[index].amountInclTax;
      this.quotation.itemList[index].description = this.document.itemList[index].description
      this.quotation.itemList[index].hsnCode = this.document.itemList[index].hsnCode
      this.quotation.itemList[index].item = this.document.itemList[index].item
      this.quotation.itemList[index].qty = this.document.itemList[index].qty
      this.quotation.itemList[index].rate = this.document.itemList[index].rate
      this.quotation.itemList[index].slno = index
      this.quotation.itemList[index].unit = this.document.itemList[index].unit


      this.quotation.itemList[index].cessAmount = this.document.itemList[index].cessAmount
      if (this.quotation.itemList[index].cessAmount == 0) {
        this.quotation.itemList[index].cessRate = 0
      } else {
        this.quotation.itemList[index].cessRate = this.document.itemList[index].cessPercentage
      }
      this.quotation.itemList[index].cgstAmount = this.document.itemList[index].cgstAmount
      if (this.quotation.itemList[index].cgstAmount == 0) {
        this.quotation.itemList[index].cgstRate = 0
      } else {
        this.quotation.itemList[index].cgstRate = this.document.itemList[index].cgstPercentage
      }
      this.quotation.itemList[index].discountedAmount = this.document.itemList[index].amountDiscounted
      if (this.quotation.itemList[index].discountedAmount == 0) {
        this.quotation.itemList[index].discountAmount = 0
        this.quotation.itemList[index].discountRate = 0
      } else {
        this.quotation.itemList[index].discountAmount = this.document.itemList[index].rateAfterDiscount
        this.quotation.itemList[index].discountRate = this.document.itemList[index].discountPercentage
      }


      this.quotation.itemList[index].igstAmount = this.document.itemList[index].igstAmount
      if (this.quotation.itemList[index].igstAmount == 0) {
        this.quotation.itemList[index].igstRate = 0
      } else {
        this.quotation.itemList[index].igstRate = this.document.itemList[index].igstPercentage
      }


      this.quotation.itemList[index].sgstAmount = this.document.itemList[index].sgstAmount
      if (this.quotation.itemList[index].sgstAmount == 0) {
        this.quotation.itemList[index].sgstRate = 0

      } else {
        this.quotation.itemList[index].sgstRate = this.document.itemList[index].sgstPercentage

      }

      this.quotation.itemList[index].vatAmount = this.document.itemList[index].vatAmount
      if (this.quotation.itemList[index].vatAmount == 0) {
        this.quotation.itemList[index].vatRate = 0
      } else {
        this.quotation.itemList[index].vatRate = this.document.itemList[index].vatPercentage
      }
    ...
    this.quotation.docData.cessValue = this.document.summaryData.cessValue;
    this.quotation.docData.cgstValue = this.document.summaryData.cgstValue;
    this.quotation.docData.discountValue = this.document.summaryData.totalAmountDiscounted
    this.quotation.docData.currency = this.document.billingAmountDetails.currency;
    this.quotation.docData.discountedAmount = this.document.summaryData.totalAfterDiscount
    this.quotation.docData.igstValue = this.document.summaryData.igstValue
    this.quotation.docData.includeCess = this.billingAmountDetailsForm.value.includeCess
    this.quotation.docData.includeDiscount = this.billingAmountDetailsForm.value.includeDiscount
    this.quotation.docData.includeTax = this.billingAmountDetailsForm.value.includeTax
    this.quotation.docData.includeUnit = this.billingAmountDetailsForm.value.includeUnit
    this.quotation.docData.interState = this.billingAmountDetailsForm.value.interState
    this.quotation.docData.sgstValue = this.document.summaryData.sgstValue
    this.quotation.docData.taxType = this.billingAmountDetailsForm.value.taxType
    this.quotation.docData.total = this.document.summaryData.total
    this.quotation.docData.totalInclTax = this.document.summaryData.totalInclTax
    this.quotation.docData.vatValue = this.document.summaryData.vatValue;
  ...
  getPrefixAndDocNumber(prefix, quoteNoLast) {
    if (!prefix) {
      prefix = '';
    }
    let docNumber = 0;
    if (quoteNoLast) {
      docNumber = quoteNoLast;
    }
    ...
    let num = docNumber + 1
    this.quotation.docData.docNumber = num.toString()
    this.quotation.docData.docPrefix = prefix
    this.quotation.docData.prefixAndDocNumber = prefix + (docNumber + 1)
  }
  ...
  updateFormDetailsFromUserdata(allData: UserDatas) {
    this.fieldNameEstimate = allData.superUserDetails.fieldNames?.fieldNameEstimate ? allData.superUserDetails.fieldNames?.fieldNameEstimate : 'Estimate'
    ...
    this.quoteOrgTag = allData.superUserDetails.quotationOrgTag ? allData.superUserDetails.quotationOrgTag : false
    this.quoteContactTag = allData.superUserDetails.quotationContactTag ? allData.superUserDetails.quotationContactTag : false
    this.quoteSaleTag = allData.superUserDetails.quotationSaleTag ? allData.superUserDetails.quotationSaleTag : false
    ...
    this.quotationAutoPayLinkEnable = (!!allData.superUserDetails.quotationAutoPayLinkEnable)
    ...
    if (this.scenario == 'create' && !this.formResetDisable) {
      ...
      this.quotation.createdBy = allData.userId;
      this.mainForm.patchValue({ docTitle: allData.superUserDetails.fieldNames?.fieldNameQuotation ? allData.superUserDetails.fieldNames?.fieldNameQuotation : 'Quotation' })
      this.docDetailsForm.patchValue({ docTitle: allData.superUserDetails.fieldNames?.fieldNameQuotation ? allData.superUserDetails.fieldNames?.fieldNameQuotation : 'Quotation' })

      this.getPrefixAndDocNumber(allData.superUserDetails.quotationNumberPrefix, allData.superUserDetails.quoteNoLast)
      this.createItemFormGroupDefault();
      if (allData.superUserDetails?.quotationApproval) {
        this.quotation.docData.statusApproved = false;
      } else {
        this.quotation.docData.statusApproved = true;
      }
      if (this.custID == 'none') {
        this.custID = null
  ...
  updateEditalue(data) {
    this.quotation.docData.docTitle = data.docData?.docTitle;
    this.quotation.createdBy = data?.createdBy;
    this.quotation.sharedDocId = data?.sharedDocId;
  ...
  // in create quo from estimate scenarion bind basic details
  updateEstimateEditalue(data, allData) {
    this.saleID = data.docData?.saleID; // bind sale id
    this.custID = data.customerData?.custID; // bind customer id
    this.orgID = data.customerData?.orgID; // bind orgID 
    this.userData = data?.userData;
    ...
    this.itemLists?.forEach((item, index) => {
      ...
      this.document.itemList[index].totalProductRate = item.amount,
        ...
        this.document.itemList[index].sgstPercentage = item.sgstRate,
        this.document.itemList[index].slno = item.slno,
        this.document.itemList[index].hsnCode = item.hsnCode,
        this.document.itemList[index].discountPercentage = item.discountRate,
        this.document.itemList[index].rateAfterDiscount = item.discountAmount,
        this.document.itemList[index].amountDiscounted = item.discountedAmount
      if (item.discountRate > 0) {
    ...
    this.docData.docType = 'Quotation'; //reset the docType to Quotation
    this.docData.estRef = data.docData.prefixAndDocNumber; // bind estref as the document number from estimate
    this.quotation.docData.estRef = data.docData.prefixAndDocNumber;
    this.docData.docValidity = null; // set validity as null
    this.docData.docDate = null; // set doc date as null
    if (!this.docData.taxType) {
      // if there is no tax type set it as gst
    ...
    this.docData.docTitle = allData.superUserDetails.fieldNames?.fieldNameQuotation ? allData.superUserDetails.fieldNames?.fieldNameQuotation : 'Quotation'
    this.getPrefixAndDocNumber(allData.superUserDetails.quotationNumberPrefix, allData.superUserDetails.quoteNoLast)
    this.createItemFormGroupDefault();
    if (allData.superUserDetails?.quotationApproval) {
      this.quotation.docData.statusApproved = false;
  ...
  formValueChanges() {
    ...
    this.mainForm.valueChanges.subscribe((data) => {
      this.quotation.docData.docTitle = this.mainForm.value.docTitle
      this.updateValidCheck()
    })
  ...
  bindAddtionalDetails() {
    ...
    this.quotation.additionalFieldsArr = []
    this.quotation.additionalFieldsArr = additionalFields
  }

  bindCustomerData() {
    this.quotation.customerData.addressline1 = this.billToForm.value.billToAddressline1;
    this.quotation.customerData.addressline2 = this.billToForm.value.billToAddressline2;
    this.quotation.customerData.companyName = this.billToForm.value.billToCompanyName;
    this.quotation.customerData.contactNumber = this.billToForm.value.billToContactNumber ? this.billToForm.value.billToContactNumber + '' : this.billToForm.value.billToContactNumber;
    this.quotation.customerData.country = this.billToForm.value.billToCountry;
    this.quotation.customerData.countryCode = this.billToForm.value.billToCountryCode;
    this.quotation.customerData.district = this.billToForm.value.billToDistrict;
    this.quotation.customerData.email = this.billToForm.value.billToEmail;
    this.quotation.customerData.fname1 = this.billToForm.value.billToFname1;
    this.quotation.customerData.gst = this.billToForm.value.billToGst;
    this.quotation.customerData.pinCode = this.billToForm.value.billToPinCode;
    this.quotation.customerData.sname = this.billToForm.value.billToSname;
    this.quotation.customerData.state = this.billToForm.value.billToState;
    this.quotation.customerData.surname = this.billToForm.value.billToSurname;
    this.quotation.searchTerm.firstName = this.billToForm.value.billToFname1 ? this.billToForm.value.billToFname1.toLowerCase() : ''
    this.quotation.searchTerm.secondName = this.billToForm.value.billToSname ? this.billToForm.value.billToSname.toLowerCase() : ''
    this.quotation.searchTerm.surname = this.billToForm.value.billToSurname ? this.billToForm.value.billToSurname.toLowerCase() : ''
    this.quotation.searchTerm.companyName = this.billToForm.value.billToCompanyName ? this.billToForm.value.billToCompanyName.toLowerCase() : ''
  }
  bindDeliveryData() {
    this.quotation.customerData.isDeliveryAddressPresent = this.deliverdToForm.value.isDeliveryAddressPresent;
    this.quotation.customerData.deliveryAddressline1 = this.deliverdToForm.value.deliveryAddressline1;
    this.quotation.customerData.deliveryAddressline2 = this.deliverdToForm.value.deliveryAddressline2;
    this.quotation.customerData.deliveryContactName = this.deliverdToForm.value.deliveryContactName;
    this.quotation.customerData.deliveryCompanyName = this.deliverdToForm.value.deliveryCompanyName;
    this.quotation.customerData.deliveryContactNumber = this.deliverdToForm.value.deliveryContactNumber ? this.deliverdToForm.value.deliveryContactNumber + '' : this.deliverdToForm.value.deliveryContactNumber;
    this.quotation.customerData.deliverycountryCode = this.deliverdToForm.value.deliverycountryCode;
    this.quotation.customerData.deliveryCountry = this.deliverdToForm.value.deliveryCountry;
    this.quotation.customerData.deliveryDistrict = this.deliverdToForm.value.deliveryDistrict;
    this.quotation.customerData.deliveryPinCode = this.deliverdToForm.value.deliveryPinCode;
    this.quotation.customerData.deliveryState = this.deliverdToForm.value.deliveryState;
    this.quotation.customerData.deliveryEmail = this.deliverdToForm.value.deliveryEmail;
  }
  bindUserAddressData(data) {
    this.quotation.userData.addressline1 = this.billFromForm.value.billFromAddressline1;
    this.quotation.userData.addressline2 = this.billFromForm.value.billFromAddressline2;
    this.quotation.userData.city = this.billFromForm.value.billFromCity;
    this.quotation.userData.companyName = this.billFromForm.value.billFromCompanyName;
    this.quotation.userData.contactname = this.billFromForm.value.billFromContactname;
    this.quotation.userData.country = this.billFromForm.value.billFromCountry;
    this.quotation.userData.gst = this.billFromForm.value.billFromGst;
    this.quotation.userData.pinCode = this.billFromForm.value.billFromPinCode;
    // this.quotation.userData.secondName = this.billFromForm.value.secondName;
    this.quotation.userData.state = this.billFromForm.value.billFromState;
  }
  bindUserData() {
    this.quotation.userData.contactno = this.signatureAndAdditionalDetailsForm.value.signatoryContactno ? this.signatureAndAdditionalDetailsForm.value.signatoryContactno + '' : this.signatureAndAdditionalDetailsForm.value.signatoryContactno;
    this.quotation.userData.designation = this.signatureAndAdditionalDetailsForm.value.designation;
    this.quotation.userData.email = this.signatureAndAdditionalDetailsForm.value.signatoryEmail;
    this.quotation.userData.signatoryName = this.signatureAndAdditionalDetailsForm.value.signatoryName;
    this.quotation.docData.bankDetails = this.signatureAndAdditionalDetailsForm.value.bankDetails;
    this.quotation.docData.notes = this.signatureAndAdditionalDetailsForm.value.notes;
  }
  bindDocData() {
    this.quotation.docData.docDate = this.docDetailsForm.value.docDate;
    this.quotation.docData.docValidity = this.docDetailsForm.value.docValidity;
    this.quotation.docData.gstPlaceOfSupplyCode = this.docDetailsForm.value.gstPlaceOfSupplyCode;
    this.quotation.docData.gstStateCode = this.docDetailsForm.value.gstStateCode;
  }
  ...
  getEstimateDocumentDetails(userId, docId) {
    return this.firestore
  ...
  createDocument(changeLog) {
    return this.firestore
      .collection('users/' + this.superUserId + '/Quotations/')
      .add({
        ...this.quotation,changeLog
      })
  }
  editDocument(changeLog) {
    if(Object.keys(changeLog).length > 0)
      return this.firestore.collection('users/' + this.superUserId + '/Quotations/')
        .doc(this.docID)
        .set({
          ...this.quotation, changeLog
        })
