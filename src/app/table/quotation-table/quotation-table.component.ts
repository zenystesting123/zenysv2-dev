/*****************************************************
Descrition : table for quotation, take data dynamically based on the page size
 * ************************************************************* */
...
import { PaginatorQuotationIntl } from './pagination-quotation-table';
...
@Component({
  ...
  providers: [
    { provide: MatPaginatorIntl, useClass: PaginatorQuotationIntl}
  ]
})
export class QuotationTableComponent implements OnInit, OnDestroy {
  ...
  tableDefaultData = QuotationTableColumns;// table columns configuration . used for adding new field which is added on modal
  displayName: string = 'displayQuotationColumns';// field for saving table settings
  userIdsArray: any[] = [];// users id
  userNamesArray: any[] = [];// users names
  customFieldQuotation: any[]; // contact additional fields
  displayedColumns = []; // for getting the selected column for displaying in the table

  // custom field names
  fieldNameQuotation: string = 'Quotation';//for set field name for quotation
  ...
  constructor(public commonService: CommonService, public tableService: QuotationTableService,
    public chageDetection: ChangeDetectorRef,
    public datepipe: DatePipe,
    private snackBar: MatSnackBar,
    public dialog: MatDialog,
    private router: Router,
    public networkCheck: NetworkCheckService, private customPaginatorIntl: PaginatorQuotationIntl) {
    this.paginatorIntl = customPaginatorIntl;
    //initialize array
    this.tableService.quotationList = new MatTableDataSource([]);
    ...
    this.commonServiceUserSubscription = this.commonService.userDatas.subscribe(
      (allData) => {
        if (!allData.userDetails.enableLiteMode) {
          this.router.navigate(['dash/documents-list/Quotationlist']);
        }
        else {
           ...
           if (allData.usrProfileData) {
            ...
            if (allData.usrProfileData.isCheckedSalesQuot == false) {
              this.disableView = true; // disable view quotation
          ...
          if(!this.disableView && this.commonService.userPlan.documentsAccess && this.commonService.userPlan.quotationsAccess){
          ...
          if (allData.superUserDetails.fieldNames) {
              this.fieldNameQuotation = allData.superUserDetails.fieldNames.fieldNameQuotation;
          }
         
          this.customFieldQuotation = allData.superUserDetails.customFieldsQuotation;
          let displayColumnsSaved: DisplayColumn[] = [];
          if (allData.userDetails.displayQuotationColumns) {
            displayColumnsSaved = allData.userDetails.displayQuotationColumns;
          }
          if (displayColumnsSaved.length > 0) {
            //if table settings are stored in db, use the stored data
            this.columnsDispaly = allData.userDetails.displayQuotationColumns;
           ...
           if (allData.superUserDetails.plan == 'invoicing'){
              ...
              this.columnsDispaly = QuotationTableColumnsInvPlan;
              ...
              this.columnsDispaly = QuotationTableColumns;
            }
          }
          ...
          if (!this.firstSetDataLoaded) {// for calling the function only once when user datas are chaged no need tocall the function again      
            if (this.tableService.filterViewSelected == "This Week") {
              this.tableService.viewSelected = "This Week " + this.fieldNameQuotation;// for displaying viewname in toolbar
            } else if (this.tableService.filterViewSelected == "This Month") {
              this.tableService.viewSelected = "This Month " + this.fieldNameQuotation;// for displaying viewname in toolbar
            }else if (this.tableService.filterViewSelected == "This Quarter") {
              this.tableService.viewSelected = "This Quarter " + this.fieldNameQuotation;// for displaying viewname in toolbar
            }else if (this.tableService.filterViewSelected == "This Year") {
              this.tableService.viewSelected = "This Year " + this.fieldNameQuotation;// for displaying viewname in toolbar
            }
  ...
  configureTable() {
    ...
    this.customFieldQuotation?.forEach((element, index) => {
      if (!element.isActive) {
    ...
    this.customFieldQuotation?.forEach((element, index) => {
      if (element.isActive) {
        let field = this.customFieldQuotation[index].fieldName;
        ...
        if (fieldPresent == false) {
          this.columnsDispaly.push({
            columnDef: this.customFieldQuotation[index].fieldName,
            header: this.customFieldQuotation[index].fieldName,
            display: false,
            type: this.customFieldQuotation[index].fieldType,
  ...
  getData(pageIndex) {
    ...
    this.getDataSubscription = this.tableService.getData(this.superUserId).subscribe(data => {
      ...
      if (data.length > 0) {
        ...
        this.tableService.quotationList = new MatTableDataSource<Invoice>(data);
        ...
        let datalist = this.tableService.quotationList.data.map((obj) => ({
          ...obj,
        }));
        ...
        this.tableService.quotationList.data = datalist;
        ...
        this.tableService.quotationList = new MatTableDataSource<Invoice>([]);
        this.tableService.lastDocumentId = docId;
        this.tableService.quotationList.data = []
  ...
  viewSelected(viewName) {
    ...
    this.tableService.quotationList.data = [];// clear data
  ...
  openDialog() {
    ...
    const dialogRef = this.dialog.open(CustomTableSettingsComponent, {
      data: {
        ...
        customFields: this.customFieldQuotation,
      },
      disableClose: true,
      width: '600px',
    });
