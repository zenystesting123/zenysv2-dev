/*---------------------------------------------------------------------------
Description: Its the parent component of calendar 


-----------------------------------------------------------------------------*/
import { ChangeDetectorRef, Component, HostListener, OnDestroy, OnInit, ViewChild } from '@angular/core';
import { MatDialog } from '@angular/material/dialog';
import { CalendarEventDetails, EventDate } from 'src/app/data-models';
...
@Component({
  selector: 'app-google-calandar-events-list',
  templateUrl: './google-calandar-events-list.component.html',
  styleUrls: ['./google-calandar-events-list.component.scss'],
})
export class GoogleCalandarEventsListComponent implements OnInit {
  addNewEvent: CalendarEventDetails; // for adding event
  ...
  constructor(
    ...
    this.googleCalendarEventService.calendarView = this.view; // passing view to service
  ...
  async onLogIn() {
    ...
    this.connectCalendarDisplay = false;
    this.onDayChange(this.viewDate);
  }
  onLogOut() {
    // gapi logout
    this.googleCalendarEventService.logout();
    this.connectCalendarDisplay = true;
  }
  onAddEvent() {
    // adding new event
  ...
  onWeekChange(viewDate: Date) {
    ...
    this.googleCalendarEventService.calendarView = this.view; //upadate view in service
    ...
    this.googleCalendarEventService.selectedWeekfirstDate = firstDay; //upadate first day of week in service
    this.googleCalendarEventService.selectedWeekLastDate = lastDay; //upadate last day of week  in service
    this.googleCalendarEventService.getWeek(); //fetch week wise event
  }
  onMonthChange(viewDate: Date) {
    ...
    firstDay.setHours(0, 0, 0);
    lastDay.setHours(23, 59, 59, 999); // set last day hour to 11:59:59:999
    this.googleCalendarEventService.selectedMonthfirstDate = firstDay; //upadate first day of month  in service
    this.googleCalendarEventService.selectedMonthLastDate = lastDay; //upadate last day of month  in service
  ...
  onDayChange(viewDate: Date) {
    ...
    this.googleCalendarEventService.selectedDay = currentDay; //upadate current day in service
    this.googleCalendarEventService.nextDay = nextDay; //upadate current day in service
    this.getSelecteDayDataFromDB(); //fetch day wise event
  }
  getSelecteDayDataFromDB() {
    this.googleCalendarEventService.getDayCalendar(); //fetch day wise event
  }
  getSelectedMothDataFromDB() {
    this.googleCalendarEventService.getMonth();  //fetch month wise event
  }
