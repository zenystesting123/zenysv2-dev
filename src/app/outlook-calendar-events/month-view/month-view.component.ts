...
export class MonthViewComponent implements OnInit {
  ...
  getDateString(rowCount: number, colCount: number) {
    ...
    var monthStartDayNumber =
      this.outlookCalendarEventService.selectedMonthfirstDate.getDay();
    var lastDate =
      this.outlookCalendarEventService.selectedMonthLastDate.getDate();
  ...
  getTaskDate(dateIndex: number): OutlookCalendarEventDetails[] {
    // get events based on the date
    dateIndex -= 1;
    let completeData: OutlookCalendarEventDetails[] =
      this.outlookCalendarEventService.selectedMonthCalendarList;
    let filterDateEvents: OutlookCalendarEventDetails[] = [];
    var day = new Date(this.outlookCalendarEventService.selectedMonthfirstDate);
  ...
  onIncrementMonth() {
    // on montyh increase
    let firstDay = this.outlookCalendarEventService.selectedMonthfirstDate;
  ...
  onMonthChange(viewDate: Date) {
    ...
    this.outlookCalendarEventService.selectedMonthfirstDate = firstDay;
    this.outlookCalendarEventService.selectedMonthLastDate = lastDay;
