...
import { AttendessEmail, OutlookCalendarEventDetails, Customer, EventDate, OverRides, ReminderDetails, SubUsers } from 'src/app/data-models';
...
@Component({
  selector: 'app-outlook-calendar-events-details',
  templateUrl: './outlook-calendar-events-details.component.html',
  styleUrls: ['./outlook-calendar-events-details.component.scss']
})
export class OutlookCalendarEventsDetailsComponent implements OnInit {
  ...
  fieldNameContact='Contact' //default name for contacts module
  ...
  constructor(
    ...
    @Optional()
    public dialogRef: MatDialogRef<OutlookCalendarEventsDetailsComponent>,
    ...
    if (this.data.scenario == 'create') {
      ...
      if (this.data.result.attendees) {
        ...
        this.data.result.attendees.forEach((element) => {
          this.emailList.push(element.emailAddress.address);
        });
      }
    }
      ...
      .subscribe((data) => {
        ...
        if (authDetails) {
          ...
          if (data.usrProfileData.dialogdataAccessRule == 'All') {
            ...
            this.outlookCalendarEventService
  ...
  onSubmit(form: NgForm, GAevent) {
    ...
    // add values to OutlookCalendarEventDetails calss
    let obj = new OutlookCalendarEventDetails();
      ...
      .then((data) => {
        ...
        if (data == true) {
          ...
          if (this.outlookCalendarEventService.calendarView == 'day') {
            ...
            // if its week view fetch week wise data
            this.outlookCalendarEventService.getWeek();
          } else if (this.outlookCalendarEventService.calendarView == 'dayMobile') {
